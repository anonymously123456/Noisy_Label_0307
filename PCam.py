...
def prepare_data(args):
    ...
    transform_test = transforms.Compose([transforms.Resize([224, 244]),
                                         transforms.ToPILImage(),
                                         transforms.ToTensor(),
                                         transforms.Normalize(mean=[0.5, 0.5, 0.5], std=[0.5, 0.5, 0.5])])

    train_file_path, train_labels_int, val_file_path, val_labels_int, test_file_path, test_labels_int, y_train_noisy, T = data_utils.PCam(
        data_path='./data/hist', dataset_list=['Noise'], noise_prob=args.noise_prob)
    ...
    train_set = data_utils.CancerDataset(
        train_file_path, y_train_noisy, transform=transform_train)
    validation_set = data_utils.CancerDataset(
        val_file_path, val_labels_int, transform=transform_val)
    test_set = data_utils.CancerDataset(
        test_file_path, test_labels_int, transform=transform_val)
...
def train(args, model, train_loader, optimizer, loss_fun, device, T, e, pre_train_e=10):
    ...
    if e >= pre_train_thre:
        ...
        y_all = y_all.numpy()
        print('=> Start EM')
        print(np.max(l_all), np.min(l_all))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--save_path', type=str,
                        default='./checkpoint/PCam', help='path to save the checkpoint')
